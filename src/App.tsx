...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {screen === 'agenda' && <section className="page"><p className="eyebrow">Confronto settimanale</p><h2>Tutti e tre i calendari</h2><p className="page-intro">Ogni scheda confronta Altino, Matese e Perugia. L’app sceglie una sola gara in casa; in caso di concomitanza vince sempre Altino.</p><div className="agenda-list">{weekends.map((weekend) => { const choice = chooseHomeMatch(weekend.matches); return <section className="week-card compare-week" key={weekend.key}><div className="week-heading"><div><strong>Settimana del {dateLabel(weekend.key)}</strong><span>Confronto casa e trasferta</span></div><span className={`choice-pill ${choice ? '' : 'free'}`}>{choice ? `VEDI ${teams.find((t) => t.id === choice.team)?.shortName.toUpperCase()}` : 'NESSUNA GARA IN CASA'}</span></div><div className="comparison-grid">{teams.map((team) => <ComparisonCell key={team.id} teamId={team.id} weekMatches={weekend.matches} choice={choice} />)}</div></section> })}</div></section>}
