...
const nav: { id: Screen; label: string; icon: string }[] = [
  { id: 'home', label: 'Home', icon: '⌂' }, { id: 'agenda', label: 'Agenda', icon: '▦' },
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      {screen === 'home' && <>
        <section className="hero-card"><p className="eyebrow light">Prossima scelta</p>{nextChoice ? <><span className="hero-date">{dateLabel(nextChoice.date)}</span><h2>Vai a {teams.find((team) => team.id === nextChoice.team)?.shortName}</h2><p>{nextChoice.opponent} · gara casalinga</p><button onClick={() => setScreen('agenda')}>Apri l’agenda</button></> : <><h2>Calendari in aggiornamento</h2><p>La prossima gara casalinga apparirà qui.</p></>}</section>
      ...
      {screen === 'agenda' && <section className="page"><p className="eyebrow">Agenda personale</p><h2>Le settimane della stagione</h2><p className="page-intro">La scelta evidenziata considera esclusivamente le gare casalinghe.</p><div className="agenda-list">{weekends.map((weekend) => { const choice = chooseHomeMatch(weekend.matches); return <section className="week-card" key={weekend.key}><div className="week-heading"><div><strong>{dateLabel(weekend.date)}</strong><span>Settimana sportiva</span></div><span className={`choice-pill ${choice ? '' : 'free'}`}>{choice ? `Vai a ${teams.find((t) => t.id === choice.team)?.shortName}` : 'Nessuna gara in casa'}</span></div>{weekend.matches.map((match) => <MatchRow key={match.id} match={match} chosen={choice?.id === match.id} />)}</section> })}</div></section>}
